...
//Call OthersList
const setPost = createAction(SET_POST, (best, normal) => ({
  best,
  normal,
}));
...
const getPostFB = () => {
  return function (dispatch, getState, { history }) {
    ...
    postDB.get("/db").then((docs) => {
      docs.data.bestPostList.forEach((list) => {
      ...
      docs.data.postList.forEach((list) => {
        let post = Object.keys(list).reduce((acc, cur) => {
          return { ...acc, [cur]: list[cur] };
        }, {});

        normal_list.push(post);
      });

      console.log("베스트", Array.from(best_list));
      console.log("노말", normal_list);

      dispatch(setPost(best_list, normal_list));
    });
  };
};
...
export default handleActions(
  {
    [SET_POST]: (state, action) =>
      produce(state, (draft) => {
        draft.post_list.best_list.push(...action.payload.best);
        draft.post_list.normal_list.push(...action.payload.normal);
        //중복 post 처리
